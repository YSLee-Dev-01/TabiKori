...
/// 플랜에 저장된 쇼핑 리스트 체크리스트 화면. PlanDetail 일자 헤더의 "쇼핑 리스트" 버튼에서 push된다.
/// 상단 +버튼으로 항목 추가, 편집 버튼으로 삭제/이름 수정이 가능하다
public struct ShoppingPlanListView: View {

    @Bindable private var store: StoreOf<ShoppingPlanListFeature>

    @FocusState private var isAddFieldFocused: Bool
    @FocusState private var isAddNoteFieldFocused: Bool

    public init(store: StoreOf<ShoppingPlanListFeature>) {
        self.store = store
    }
    ...
    public var body: some View {
        ...
        .navigationTitle(Strings.Shopping.planListTitle)
        ...
        .toolbar {
            ...
            if self.store.isAdding == false {
                ToolbarItem(placement: .topBarTrailing) {
                    ...
                    .tint(Color.getTabiColor(.tabiPrimary))
                    .accessibilityLabel(Strings.Shopping.planListEditButtonAccessibilityLabel)
...
private extension ShoppingPlanListView {
    @ViewBuilder
    func content() -> some View {
        if self.store.isLoading {
            ...
            TabiEmptyState(
                ...
                title: Strings.Shopping.planListSavedEmptyTitle,
                description: Strings.Shopping.planListSavedEmptyDescription,
                style: .card
            )
    ...
    func list() -> some View {
        List {
            Section {
                ...
                // 유지한 채 ShoppingPlanListItemRow 내부에서만 isEditing에 따라 표시를 바꾼다
                ForEach(self.$store.items) { $item in
                    ShoppingPlanListItemRow(item: $item, isEditing: self.store.isEditing, isAdding: self.store.isAdding) {
                ...
                if self.store.isAdding {
                    TabiTextField(
                        placeholder: Strings.Shopping.planListAddItemPlaceholder,
                ...
                TabiLabel(
                    title: Strings.Shopping.planListCheckedCountTitle(self.store.checkedCount, self.store.items.count),
                    style: .captionMBold,
...
#Preview {
    let mockUseCase: TestShoppingPlanItemUseCase = {
        let useCase = TestShoppingPlanItemUseCase()
        useCase.savedItems = [
            ShoppingPlanItem(id: UUID(), planId: TravelPlan.mock.id, order: 0, title: "お土産", note: "職場用に個包装のもの", isChecked: true),
            ShoppingPlanItem(id: UUID(), planId: TravelPlan.mock.id, order: 1, title: "コスメ", note: nil, isChecked: false)
        ]
        return useCase
    }()

    NavigationStack {
        ShoppingPlanListView(
            store: Store(
                initialState: ShoppingPlanListFeature.State(plan: .mock),
                reducer: { ShoppingPlanListFeature() },
                withDependencies: { dependency in
                    dependency.shoppingPlanItemUseCase = mockUseCase
                }
            )
        )
    }
