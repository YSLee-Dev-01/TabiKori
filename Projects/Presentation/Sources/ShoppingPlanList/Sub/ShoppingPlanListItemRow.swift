...
/// 쇼핑 리스트 항목 행. 편집모드 여부에 따라 같은 셀 안에서 체크 표시 ↔ 텍스트필드 편집으로 내용만 전환된다.
/// 체크 행과 편집 행을 별개의 ForEach로 분리하면 List가 셀 재사용 시 일부 행에 새 콘텐츠를 반영하지 못하는
/// 문제가 있어, 하나의 행 타입/식별자를 유지한 채 내부에서만 분기한다
struct ShoppingPlanListItemRow: View {
    @Binding var item: ShoppingPlanItem
    let isEditing: Bool
...
private extension ShoppingPlanListItemRow {
    ...
    func editContent() -> some View {
        VStack(alignment: .leading, spacing: 6) {
            TabiTextField(placeholder: Strings.Shopping.planListAddItemPlaceholder, text: self.$item.title)
